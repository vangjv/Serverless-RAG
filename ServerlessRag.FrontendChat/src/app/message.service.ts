...
export class MessageService {
  ...
  sendMessage(prompt: string, orgId:string): void {
    ...
    this.getChatResponseStream(prompt, orgId).subscribe({
      next: (message) =>
  ...
  private getChatResponseStream(prompt: string, orgId:string): Observable<Message> {
    const id = window.crypto.randomUUID();

    return this.http
        .post(`${environment.ragBaseURL}/api/rag`, {"userMessage":prompt, "orgId": orgId}, {
        responseType: 'text',
        observe: 'events',
        reportProgress: true,
      })
